...
- `src/moravec_analysis/synth.py` — a synthetic `trial_results` generator with the paper's known effects baked in. Every notebook can flip `USE_SYNTHETIC` back to `True` to fall back to this when real data is too thin for a given analysis (see `02_regressor_comparison.ipynb`) — nothing else changes, since `synth.generate_synthetic_trials()` returns the same shape as `data.load_trial_results()`.
