...
**Sync**:
Reconciling a User's local progress with the backend — active for any session, anonymous or logged in. A single `POST /sync` call carries both directions at once: it pushes each pending Trial's result (the payload includes full per-keystroke timing, which is research signal in its own right, not just an anti-cheat measure) and, in the same round trip, pulls back anything new since the caller's cursor — a `sync_log`-backed sequence number, not a timestamp, since client clocks aren't trustworthy. Level and Practice trials share one backend table (`trial_results`, discriminated by a `run_type` column) but stay isolated everywhere it matters: only Level trials feed `level_runs` or the Levels unlock cache — a Practice trial is never more than a raw synced trial row. `level_runs` is only ever derived server-side from re-validated trials and returned on pull, never accepted from a request. The backend no longer stores a best-record LevelStats snapshot of its own (there is no `level_stats` table) — LevelStats is derived client-side, on every read, from a local TinyBase `levelRuns` table (IndexedDB-backed — see `apps/frontend/src/storage/store.ts` — so it survives a reload, unlike the `localStorage` it replaced) using the same better-record comparison it's always used. `apps/frontend/src/sync/syncEngine.ts`'s `sync(authState)` is what actually calls `POST /sync`: it reads every locally-unsynced trial, pushes them, and applies the pull response — trials and level runs alike — via unconditional `setRow`, with its own in-memory exponential backoff retrying a failure without needing a page reload. It's wired to fire after a Level finishes or a Practice session stops. A few more trigger points (retrying sooner on an `online` event, one attempt at app boot, a flush right after OTP login) are still pending — check for those specific call sites before assuming the full reconnect story is live.
...
**Level content lives in the backend, not a static frontend map**: each Level's weighted Operation-category mix is a `levels` table row, seeded once from a fixture on first boot and then the live source of truth — changing a Level no longer needs a frontend rebuild. `GET /levels` (the number list, for Home's grid) and `GET /levels/:levelNumber` (one Level's full mix) are both public, unauthenticated. The per-Trial random draw (`createRandomOperation`) still runs entirely client-side from whatever mix was already fetched — no round-trip per Trial, only the lookup moved server-side.

**A Level's mix is fetched client-side, with an offline cache fallback**: `/level/[levelNumber]/page.tsx` is a thin Server Component that only validates the route param; the actual `GET /levels/:levelNumber` fetch happens in `LevelPageClient` (a Client Component, via `storage/levelCache.ts`'s `fetchLevelWithFallback`), not server-side. This is necessary, not just a style choice — the fallback needs the IndexedDB-backed local store, which a Server Component can't reach. On success the mix is cached in TinyBase's `levels` table, keyed by level number; on any failure other than a genuine 404, `fetchLevelWithFallback` returns the last-cached mix instead of failing, so a previously-visited Level stays playable offline. A `levels` table row is reference-data cache, not player-generated data like `trials`/`levelRuns` — see `storage/store.ts`.

**The whole Level catalog is warmed into the cache once per session, not just visited Levels**: `AuthBoot` also calls `storage/levelCache.ts`'s `warmLevelCache()` unconditionally on mount (not gated on `ensureSession`, since `GET /levels/all` is public) — it fetches every Level's mix in one request (backend: `getAllLevels`/`GET /levels/all`, added alongside the existing per-Level `GET /levels/:levelNumber` specifically so this warm-up isn't ~150 separate round trips) and caches each one via the same `cacheLevel` that `fetchLevelWithFallback` already uses. This is what actually closes the gap the per-Level cache alone left open: a Level the player has never played can still be started offline, as long as the device was online for at least one earlier session. `warmLevelCache()` never rejects (best-effort — a failure just leaves whichever Levels weren't already cached to fall back lazily, same as before this existed) and only runs on mount, not on the `online` reconnect trigger — the catalog is rarely-written reference data (see backend `db.ts`), so re-fetching all ~150 levels on every reconnect blip isn't worth it.
